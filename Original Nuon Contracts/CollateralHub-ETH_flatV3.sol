...
// File: contracts/Nuon/Pools/CollateralHub-ETH.sol
...
*/
contract CollateralHubETHV3 is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    */
    function initialize(
        ...
        collateralToNuonRoute = [collateralUsed,USDT,NUON];
        collateralToPairRoute = [collateralUsed,USDT];
    ...
    function setCoreAddresses(
        ...
        address _truflation,
        address _ChainlinkOracle,
        address[] memory _collateralToPairRoute
    ) public onlyOwner {
        Treasury = _treasury;
        ...
        ChainlinkOracle = _ChainlinkOracle;
        collateralToPairRoute = _collateralToPairRoute;
    }
    ...
    function setLiquidityParams(
        ...
        uint256 _liquidationFee) public onlyOwner {
        ...
        require(_liquidationFee <= 900,"Too high");
        ...
        returns (uint256)
    {
        if (viewUserCollateralAmount(_user) > 0) {
            ...
            uint256 mintedValue = viewUserMintedAmount(_user) * getTargetPeg() / 1e18;
        ...
        )
    {
        ...
        // if one of these conditions true, will return all of your collateral.
        ...
        returns (uint256)
    {
        ...
        emit MintedNUON(msg.sender, NUONAmountD18,getTargetPeg(),collateralAmount);
    ...
    function _addLiquidity(uint256 _amount) internal {
        ...
        // CollateralHub is approving the Uniswap contract to spend the collateralUsed(ERC-20
        // ..WETH contract) WETH.
        IERC20Burnable(collateralUsed).approve(router, _amount);
        // Get current CollateralHub balance of NUON tokens.
        uint256 balBefore = IERC20Burnable(NUON).balanceOf(address(this));
        // Swaps coll WETH for USDT. then USDT for Nuon.  This contract earns Nuon.
        ...
        // Swaps other half of coll WETH for USDT. This contract earns USDT.
        uint256 balBeforeusdt = IERC20Burnable(USDT).balanceOf(address(this));
        IUniswapRouterETH(router)
            .swapExactTokensForTokensSupportingFeeOnTransferTokens(
                _amountDiv2,
                0,
                collateralToPairRoute,
                address(this),
                block.timestamp
        );
        uint256 balAfterusdt = IERC20Burnable(USDT).balanceOf(address(this));

        // So we've if _amount was 6 collateral.  half sent for Nuon, half for USDT.

        // Now, approve Uniswap to spend the Nuon and USDT we earned.
        IERC20Burnable(NUON).approve(router, balAfter.sub(balBefore));
        IERC20Burnable(USDT).approve(router, balAfterusdt.sub(balBeforeusdt));

        // Add the Nuon and USDT we got into Uniswap pool as a pair.
        uint256 balOfLPBefore = IERC20Burnable(lpPair).balanceOf(address(this));
        IUniswapRouterETH(router).addLiquidity(
            USDT,
            NUON,
            balAfterusdt.sub(balBeforeusdt),
        ...
        // Burn this contract's remaining Nuon.
        IERC20Burnable(NUON).burn(IERC20Burnable(NUON).balanceOf(address(this)));
        // See balance of liquidity Pool pair token balance associated with this contract. 
        // .. which is the USDT and Nuon we sent it.
        uint256 balOfLPAfter = IERC20Burnable(lpPair).balanceOf(address(this));
        // This contract approves Relayer/Vault to spend amount of pair token/contract this contract has.
        IERC20Burnable(lpPair).approve(Relayer, balOfLPAfter.sub(balOfLPBefore));
        // Call relayer's depositOnVault(), THIS sends out pair tokens to the Vault. The Vault deposits() this
        // pair tokens in a Compounder contract in order to try to earn yield.  Then that amount, plus
        // earned interest from the Compounder, gets returned to the vault, then to us as shares.
        ...
        nonReentrant
    {
        ...
        if(getUserLiquidationStatus(msg.sender)) {
            ...
            IERC20Burnable(NUON).burn(nuonAmount);
            require(collateralPercentToRatio(msg.sender) >= INUONController(NUONController).getMaxCratio(address(this)),"Collateral Ratio too low");
            emit burnedNuon(msg.sender, nuonAmount);
        }
    }
    ...
    function liquidateUserAssets(address _user) public {
        ...
        // Protocol buys and burns Nuon to reduce the Supply.
    ...
    function calcOverCollateralizedRedeemAmounts(
        ...
        uint256 NUONAmountToBurn = (NUONValueNeeded.mul(multiplier).div(collateralPrice));
    ...
    function getLPValueOfUser(address _user) internal view returns (uint256) {
        ...
        uint256 collateralBal = IERC20Burnable(USDT).balanceOf(lpPair);
        ...
        return lpValue.mul(1e12).mul(1e18).div(getCollateralPrice());
    }
