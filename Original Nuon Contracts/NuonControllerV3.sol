...
contract NUONControllerV3 is Ownable {
    ...
    address public chainlinkUSDTFeed;
    ...
    function setChainlinkUSDTFeed(address _chainlinkUSDTFeed) public onlyOwner {
        require(_chainlinkUSDTFeed != address(0),"No 0 addresses");
        chainlinkUSDTFeed = _chainlinkUSDTFeed;
    }
    
    function setOracleAddress(address _oracle) public onlyOwner {
        require(_oracle != address(0),"No 0 addresses");
        NuonOracleAddress = _oracle;
    }
        ...
        returns (uint256)
    {
        ...
        if (NuonOracleAddress == address(0)) {
            ...
            // THINK THIS Oracel contract is deployed FIRST.  On deployment, it takes the NUON and USDT ERC-20's,
            // and creates an a pair contract. Which is what we addLiquidity to in our hub.
            // THINK the lpPair in the hub is this contract.
            // SO, price we get is based on factors including reservrs of the pair, and more.
            assetPrice = IUniswapPairOracle(NuonOracleAddress).consult(address(NUON),1e18);
            uint256 usdtPrice = IChainlinkOracle(chainlinkUSDTFeed).latestAnswer().mul(1e10);
